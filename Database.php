...
class Database {
    ...
    private function __construct($configDB) { //так как этот метод/конструктор приватный, то доступ к нему возможен только в этом классе
        try {
    ...
    public static function getInstance($configDB) {
        if (!isset(self::$instance)) {        //если переменной не существует то создается класс, иначе возвращается созданный класс
            self::$instance = new Database($configDB); //создание экземпляра/объекта класса и установка его в переменную $instance
        }
